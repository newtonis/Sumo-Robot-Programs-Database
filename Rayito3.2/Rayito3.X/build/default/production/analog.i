...
# 53 "C:/Program Files/Microchip/MPLABX/v6.00/packs/Microchip/PIC18Fxxxx_DFP/1.3.36/xc8\pic\include\proc\pic18f4550.h"
...
# 15 "eeprom_functions.h"
typedef unsigned char uc;


void WriteMem(uc addr,uc data);
void ReadMem(uc addr,uc *data);
...
for (i = 0;i < 5;i++) maxs[ls[i]] = maxs[ls[i]]>an[ls[i]]?maxs[ls[i]]:an[ls[i]];
for (i = 0;i < 5;i++) mins[ls[i]] = mins[ls[i]]<an[ls[i]]?mins[ls[i]]:an[ls[i]];

for (i = 0;i < 2;i++) maxs[es[i]] = maxs[es[i]]>an[es[i]]?maxs[es[i]]:an[es[i]];
for (i = 0;i < 2;i++) mins[es[i]] = mins[es[i]]<an[es[i]]?mins[es[i]]:an[es[i]];
}
...
j = ls[i];
aux = an[j];
aux = aux>mins[j]?aux:mins[j];
aux = aux<maxs[j]?aux:maxs[j];
